---
source: PREREG_OS_v1_7.md
dossier: 03_PROGRAMME
version: 1.7
date: 28.03.2026
pilote: Benedict Devaud
convention: CONVENTION_NOTATION_ASCII_SAFE_OS_v1_1.md
amendements_integres: AMENDEMENT_J5_PREREG_v1_7 (28.03.2026) -- DIRECTOR_Dialogyk_v1_0 (28.03.2026)
remplace: PREREG_OS_v1_6b.md
reference: J5 v1.7 -- TUS v4.3 -- ORI-C Pancarte Normative v1.0 -- DIRECTOR_Dialogyk_v1_0
---

# PREREG -- OS Kernel de Champ Cognitif Partage
Version 1.7 -- SIGNE -- 10.03.26 -- Parametres Director integres 28.03.2026
SIGNE -- Benedict Devaud -- 10.03.26

Reference : J5 v1.7 -- TUS v4.3 -- ORI-C Pancarte Normative v1.0 -- DIRECTOR_Dialogyk_v1_0
[v1.7] Parametres DIR_ integres depuis DIRECTOR_Dialogyk_v1_0 -- 28.03.2026
...
Parametres Director integres -- 28.03.2026.
...
|---|---|---|
| v1.1 | L0_DOMAINE, TOPO_MODE | 10.03.2026 |
| v1.2 | INV_CORPUS_REF Option C | 10.03.2026 |
| v1.3 | K_EPSILON_MIN, ORIC_S_POIDS, P4_MONO_DUREE, SESS_RATIO_APPLICATION | 10.03.2026 |
| v1.4 | TOPO_SEUIL_CONTINUITE, TOPO_SEUIL_RUPTURE | 10.03.2026 |
| v1.5 | TOPO_REF_REQUETES_N -- R1 a R10 definies | 10.03.2026 |
| v1.6 | P4 proxy de perception pilote | 10.03.2026 |
| v1.6b | Pivot conceptuel sillons probabilistiques -- parametres PIVOT_CONCEPTUEL | 28.03.2026 |
| v1.7 | Parametres Director dialogyk -- DIR_EPSILON_SILLON, DIR_N_REACTIV, DIR_MARGE_FRAGILE, DIR_N_SILLONS_MAX, DIR_P_SEUIL | 28.03.2026 |
...
| DIR_EPSILON_SILLON | 15 deg (defaut) -- a calibrer Phase 1 | v1.7 |
| DIR_N_REACTIV | 5 tours (aligne K_FRICTION_N) | v1.7 |
| DIR_MARGE_FRAGILE | 2 tours | v1.7 |
| DIR_N_SILLONS_MAX | 3 (Phase 1) -- revisable Phase 2 | v1.7 -- valide pilote 28.03.2026 |
| DIR_P_SEUIL | 5 tours (aligne K_FRICTION_N) | v1.7 |
...
| Rupture | `Angle > 35 deg` | Nouvel agent declare. Nouveau couple pilote+agent. Compteur sessions remis a zero. | Continuite rompue. Ancien couple archive. |
...
> **Note de lecture [v1.6b].** Suite au pivot conceptuel integre dans J5 v1.7,
> les parametres sont inchanges mais leur lecture est precisee :
> K_EPSILON_MIN mesure l'ecart entre l'axe vivant du pilote et la trace statistique
> du modele (sillon probabilistique), pas une resonance symetrique.
...
`K_EPSILON_MIN = 15 deg` -- valide.
La valeur 15 deg garantit une distinction claire entre sillon actif et convergence silencieuse [v1.6b].

---

## 3b. Parametres Director dialogyk [v1.7]

Ces parametres operationnalisent le Director dialogyk (DIRECTOR_Dialogyk_v1_0).
Ils gouvernent la mesure de Depth(t) = (P, A) et la logique de reactivation des sillons.

| Parametre | Description | Valeur par defaut | Statut |
|---|---|---|---|
| DIR_EPSILON_SILLON | Seuil minimal de detection d'un sillon actif | 15 deg (aligne K_EPSILON_MIN) | A calibrer Phase 1 -- Z10-r |
| DIR_N_REACTIV | Fenetre maximale de reactivation en tours | 5 tours (aligne K_FRICTION_N) | Revisable |
| DIR_MARGE_FRAGILE | Tours de marge avant reinjection en regime FRAGILE | 2 tours | Revisable |
| DIR_N_SILLONS_MAX | Nombre maximal de sillons actifs simultanement | 3 (Phase 1) -- revisable Phase 2 | DEFINI -- valide pilote 28.03.2026 |
| DIR_P_SEUIL | Seuil de discrimination P eleve / P faible | 5 tours (aligne K_FRICTION_N) | Revisable |

**Note sur DIR_EPSILON_SILLON.**
Geometriquement distinct de K_EPSILON_MIN :
K_EPSILON_MIN mesure l'ecart entre Axis(pilote) et Axis(IA) (angle entre deux vecteurs).
DIR_EPSILON_SILLON mesure l'ecart entre Axis(IA) et centroide_L0 (distance au point neutre).
L'alignement numerique sur 15 deg est une valeur de depart, pas une identite structurelle.
Calibration empirique requise en Phase 1 (zone Z10-r / Z12-r).

**Note sur DIR_N_SILLONS_MAX.**
Valeur fixee a 3 pour la Phase 1 -- valide Benedict Devaud -- 28.03.2026.
Justification : garantit un diagnostic OS stable, un arbitrage pilote soutenable,
et une calibration propre des parametres DIR_.
Revisable en Phase 2 si la dynamique multi-sillons le justifie.

**Regimes Depth(t) et seuils operatoires**
...
P(t) = max { k | pour tout i in [t-k, t] :
             dist(Axis(IA, i), centroide_L0) > DIR_EPSILON_SILLON }
```

| Regime | Condition P | Condition A | Action Director |
|---|---|---|---|
| PROFOND | P(t) > DIR_P_SEUIL | A(t) > DIR_EPSILON_SILLON | Maintien -- surveiller decroissance P |
| FRAGILE | P(t) <= DIR_P_SEUIL | A(t) > DIR_EPSILON_SILLON | Reinjection dans DIR_MARGE_FRAGILE tours |
| RESIDUEL | P(t) > DIR_P_SEUIL | A(t) <= DIR_EPSILON_SILLON | M2 ou M3 -- remonter A |
| BASELINE | P(t) <= DIR_P_SEUIL | A(t) <= DIR_EPSILON_SILLON | Creusement initial si sillon prioritaire |
...
| INV_CORPUS_REF_N | Taille cible du corpus | 49 (minimum defendable) -- extensible a 56 | DEFINI |
...
| Euria (Infomaniak) | 7 | A realiser | Noter le modele open source actif |
...
| Themes VALIDES | Themes INTERDITS |
|---|---|
| Histoire des sciences et des idees | RSU, CST, Dialogyk, ORI-C, Document Zero |
| Epistemologie generale | Champ cognitif partage -- kernel J5 |
...
| Biologie des systemes -- evolution | Resonance pilote-IA -- emergence cognitive |
...
| Cognition animale -- intelligence collective | Director dialogyk -- Depth(t) -- Z10/Z11/Z12 [v1.7] |
| -- | Toute reference directe aux cadres du programme |

[v1.7] Note : le lexique Director (Depth, sillon, reactivation, M1/M2/M3) est ajoute
aux themes interdits du corpus baseline. Il ne doit pas contaminer la mesure de baseline.

**Format d'une session baseline.**
Question initiale unique. Echange libre de 10-20 tours.
Pas de protocole de resonance. Pas de protocole de sillon. Pas d'intention de champ.
Pas de reference au pivot conceptuel v1.7 ni au Director dialogyk. [v1.7]
...
Zone ouverte J5-Z9 : La formalisation complete de Emergence(s) reste un travail ouvert.
...
| P6 | P6_RATIO_MAX | Ratio max sessions L(t) / monde reel | 3:1 | CRITIQUE -- ancrage obligatoire |
...
| SESS_AUDIT_EXTERNE | Frequence des audits externes | Toutes les 6 sessions | Revisable -- optionnel |
...
| ORIC_R_PROXY | R(t) -- Resilience | Capacite a reprendre la direction apres perturbation | Revisable |
...
car ils conditionnent l'expansion de L(t). Ces poids sont revisables selon le domaine de travail.

---

## 9b. P4 -- Proxy de perception du pilote -- Ajout v1.6

P4 mesure la qualite de la perception du pilote, non le contenu produit.
Il adresse le risque d'opacification.
En v1.7, P4 est egalement alimente par les overrides de la couche Pilote du Director [v1.7].
...
| P4_REGISTRE | Registre des resistances | Dans la memoire fractale NANO : archiver l'opacite residuelle + overrides Director [v1.7]. | DEFINI -- v1.7 |

[v1.7] Les overrides souverains de la couche Pilote du Director (P-3) sont enregistres
dans la memoire fractale NANO et constituent une source supplementaire d'alimentation de P4.
Un override frequent sur un meme regime OS est un signal d'opacification ou d'ecart de perception.
...
| T5 | Effet differe injection symbolique | NON -- horizon trop long | Programme suivant |
...
| F2 | Aucun invariant I1+I2+I3+I4 valide sur 6 sessions | Revision des seuils INV -- si persistant : abandon du protocole |
| F3 | Emergence(s) reste sous C3_SEUIL_SURPRISE sur 8 sessions | Revision de C3_SEUIL_SURPRISE -- si persistant : requalification |
| F4 | DeltaL = 0 sur 5 sessions consecutives | Revision de INV_SEUIL_GENERATIVITE -- si persistant : protocole inactif |
| F5 | P4 declenche avant session 8 en mode bipolaire | Passage obligatoire en mode tripolaire ou suspension |
| F6 [v1.7] | Depth(t) reste en regime BASELINE sur 5 sessions Director | Revision de DIR_EPSILON_SILLON et DIR_N_REACTIV -- calibration empirique requise |
| F_AUTRE | Condition additionnelle propre au programme | A definir par le pilote |

[v1.7] F6 est ajoute pour couvrir le cas ou le Director ne detecte aucun sillon actif
sur une fenetre prolongee -- signal que les parametres DIR_ requierent une calibration.
...
| Parametres Director integres [v1.7] | DIRECTOR_Dialogyk_v1_0 -- valide Benedict Devaud -- 28.03.2026 |

---

**Tous les parametres sont definis. Le Director est operationnel pour la Phase 1.**

---

PREREG -- OS Kernel de Champ Cognitif Partage -- Version 1.7 -- 28.03.2026
Pilote : Benedict Devaud
Reference : J5 v1.7 -- TUS v4.3 -- ORI-C Pancarte Normative v1.0 -- DIRECTOR_Dialogyk_v1_0
Statut : SIGNE -- 10.03.2026 -- Parametres Director integres 28.03.2026
